...
export class CommissionRulesController {
  ...
  create(@Request() req, @Body() body: any) { return this.svc.create(req.user.tenantId, body, req.user.id); }

  @Patch(':id')
  @Roles(...ROLE_GROUPS.MANAGEMENT)
  update(@Request() req, @Param('id') id: string, @Body() body: any) { return this.svc.update(req.user.tenantId, id, body, req.user.id); }

  @Delete(':id')
  @Roles(...ROLE_GROUPS.MANAGEMENT)
  remove(@Request() req, @Param('id') id: string) { return this.svc.remove(req.user.tenantId, id, req.user.id); }
}
